...
user_input = "norway" # TODO: global variables are considered bad practice ...

def getPlacesBasedOnUserCommand():
    places = []
    if len(sys.argv[1:]) > 0: 
        global user_input
        user_input = sys.argv[1:][0].lower()
        if not user_input == 'oslo' and not user_input == 'bergen':
        ...
        if user_input == 'bergen': places = bergen_places
        elif user_input == 'oslo': places = oslo_places
    return places

def getData(year, ws, places):    
...
def drawGraph(data):
    ...
    plt.xticks(x, ticks)
    plt.title('Average daily traffic by month from the NPRA 2002-2015 in ' + user_input.title())
...
def main():
    ...
    for i in range(2002, 2016): # data tilgjengelig år 2002-2015
        data_years.append(getData(i, ws, getPlacesBasedOnUserCommand()))
